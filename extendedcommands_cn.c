...
int show_install_update_menu() {
    ...
    static const char* headers[] = {  "安装ZIP", "", NULL};
    ...
    install_menu_items[FIXED_TOP_INSTALL_ZIP_MENUS + num_extra_volumes]     = "从上次安装目录选择ZIP";
    install_menu_items[FIXED_TOP_INSTALL_ZIP_MENUS + num_extra_volumes + 1] = "从adb sideload选择ZIP";
    install_menu_items[FIXED_TOP_INSTALL_ZIP_MENUS + num_extra_volumes + 2] = "签名验证开关";
...
static void show_choose_zip_menu(const char *mount_point) {
    ...
    static const char* headers[] = {  "选择安装ZIP", "", NULL};
...
int confirm_selection(const char* title, const char* confirm) {
    ...
    const char* confirm_headers[]  = {  title, "  该操作无法取消.", "", NULL };
...
int format_unknown_device(const char *device, const char* path, const char *fs_type) {
    ...
    if (0 == strcmp(path, "/sd-ext")) {
        ...
        if (vol == NULL || 0 != stat(vol->blk_device, &st)) {
            LOGI("没有找到app2sd分区，跳过格式化 /sd-ext.\n");
            return 0;
        }
    }
...
int show_partition_menu() {
    static const char* headers[] = { "挂载/存储", "", NULL };
...
static void choose_default_backup_format() {
    static const char* headers[] = { "默认备份文件格式设置", "", NULL };
...
int show_nandroid_menu() {
    ...
    static const char* headers[] = { "备份/恢复", "", NULL };
...
int show_advanced_menu() {
    ...
    static const char* headers[] = { "高级功能", "", NULL };
    ...
    if (!strcmp(bootloader_mode, "download")) {
        ...
        list[list_index++] = "重启到Fastboot";
    }

    list[list_index++] = "关机";
    list[list_index++] = "清除虚拟机缓存";
    list[list_index++] = "错误报告";
...
void handle_failure(int ret) {
    ...
    ui_print("/tmp/recovery.log 已复制到 /sdcard/clockworkmod/recovery.log. 请打开 ROM Manager 以报告问题.\n");
}
